...
@Component({
  selector: 'app-experiential',
  templateUrl: './experiential.component.html',
  styleUrls: ['./experiential.component.scss']
})
export class ExperientialComponent {
  loading = false;
  ...
  ngOnInit(): void {
    this.loading = true;
    this.error = null;
    this.commonApi.allExperientialApi(this.page, this.limit).subscribe({
      next: response => {
        this.loading = false;
        this.error = null;
        this.data = response
