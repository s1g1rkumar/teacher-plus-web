...
@Component({
  selector: 'app-behaviorist-assessment-evaluate',
  templateUrl: './behaviorist-assessment-evaluate.component.html',
  styleUrls: ['./behaviorist-assessment-evaluate.component.scss']
})
export class BehavioristAssessmentEvaluateComponent {
  loading = false;
  ...
  ngOnInit() {
    ...
    this.error = null;
    if (this.form_id) {
      this.commonApi.getBehavioristApi(this.form_id).subscribe({
        next: response => {
          this.loading = false;
          this.error = null;
          this.data = response;
